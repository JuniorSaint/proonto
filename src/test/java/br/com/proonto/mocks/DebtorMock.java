package br.com.proonto.mocks;

public interface DebtorMock {
    public static final String NAME_DEBTOR = "John Doe";
    public static final String CNPJCPF_DEBTOR = "882.882.882-12";
    public static final String ISSUING_AGENCY = "SSP-MG";
    public static final String NATIONALITY = "Brasileira";
    public static final String MARITAL_STATUS = "comunhão de bens";
    public static final String PROPERTY_REGIME = "regime";
    public static final String PROFESSION = "médico";


}
